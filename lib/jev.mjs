...
import { chmodSync, mkdirSync, readFileSync, writeFileSync, appendFileSync, existsSync } from "node:fs";
import { homedir } from "node:os";
import { join } from "node:path";
import { BrokerUnavailable, brokerPost, spawnBroker } from "./broker.mjs";
import { redact, redactDeep } from "./redact.mjs";

export const MODES = new Set(["off", "shadow", "active"]);
const OPT_IN = new Set(["RULES", "SCOPE", "BASH"]);
...
export function readConfig(env = process.env) {
  ...
  return {
    ...
    // Warm connection broker (lib/broker.mjs). Off means every call opens its own connection.
    broker: env.JEV_GATES_BROKER !== "off",
    // The API keeps an idle connection at least 13 minutes (measured); an hour covers breaks.
    brokerIdleMs: positive(env.JEV_GATES_BROKER_IDLE_MS, 3_600_000),
    ...
    // A sentence at or above this is part of text drafted for someone else, so not a request.
    // Real requests scored 0.07-0.09 here, a drafted question 0.62; the cutoff sits between.
    draftThreshold: unit(env.JEV_GATES_DRAFT_THRESHOLD, 0.35),
    ...
    // Intent guard, and the gap check that shares its request
    intentThreshold: unit(env.JEV_GATES_INTENT_THRESHOLD, 0.8),
    checkThreshold: unit(env.JEV_GATES_CHECK_THRESHOLD, 0.3),
    followUpThreshold: unit(env.JEV_GATES_FOLLOWUP_THRESHOLD, 0.4),
    // Bash guard
    bashThreshold: unit(env.JEV_GATES_BASH_THRESHOLD, 0.7),
    // Commit guard
    commitThreshold: unit(env.JEV_GATES_COMMIT_THRESHOLD, 0.3),
    // Per-gate switches: JEV_GATES_DONE, _CLAIMS, _PROOF, _INTENT, _CHECK, _COMMIT = off.
    // Rules, scope and bash are opt-in (JEV_GATES_RULES=on, _SCOPE=on, _BASH=on): they ask before a
    // tool runs, and in real use the first two interrupted far more often than they caught anything.
    gates: Object.fromEntries(["RULES", "SCOPE", "BASH", "DONE", "CLAIMS", "PROOF", "INTENT", "CHECK", "COMMIT"].map((g) => [g.toLowerCase(), OPT_IN.has(g) ? env[`JEV_GATES_${g}`] === "on" : env[`JEV_GATES_${g}`] !== "off"])),
    // Storage
...
export async function readStdinJson(timeoutMs = 3000) {
  const chunks = [];
  // Cleared once stdin ends: a pending timer kept every hook alive for the full
  // timeout after its work was done, and Claude Code waits for the exit.
  let handle;
  const timer = new Promise((resolve) => (handle = setTimeout(() => resolve(undefined), timeoutMs)));
  const read = (async () => {
...
export async function ask({ config, key, state, questions }) {
  // Secrets never leave the machine, whatever the gate put in its state.
  const bodyText = JSON.stringify({ model: config.model, state: redactDeep(state), questions });
  const started = performance.now();
  const response = await post(config, key, bodyText);
  const latency_ms = Math.round(performance.now() - started);
  if (response.status < 200 || response.status >= 300) throw new Error(`TypeSafe returned HTTP ${response.status}`);
  const body = JSON.parse(response.text);
  const answers = {};
  for (const [id, question] of Object.entries(questions)) {
    answers[id] = validateAnswer(id, question, body?.answers?.[id]);
  }
  return { answers, latency_ms, usage: body.usage ?? {}, model: body.model, via: response.via };
}

/** POST through the warm broker when one is running, else directly. */
async function post(config, key, bodyText) {
  const started = Date.now();
  if (config.broker) {
    try {
      return { ...(await brokerPost(config, key, bodyText)), via: "broker" };
    } catch (err) {
      if (!(err instanceof BrokerUnavailable)) throw err;
      // No broker, or a stale one on its way out: start one for the next call.
      if (/ENOENT|ECONNREFUSED|stale/.test(err.message)) spawnBroker(config);
    }
  }
  // Whatever the broker spent comes out of the direct call's budget.
  const timeoutMs = Math.max(1000, config.timeoutMs - (Date.now() - started));
  return { ...(await directPost({ ...config, timeoutMs }, key, bodyText)), via: "direct" };
}

async function directPost(config, key, bodyText) {
...
// Private to this user. `mode` only applies on creation, so existing paths from
// older versions are tightened on every write too.
...
export function log(config, fields) {
  try {
    ...
    const line = [new Date().toISOString(), ...fields.map((f) => redact(String(f ?? "")).replace(/[\t\n\r]+/g, " "))].join("\t");
    const path = join(config.dataDir, "decisions.log");
...
export function writeLast(config, name, payload) {
  try {
    ensureDir(config.dataDir);
    privateWrite(join(config.dataDir, `last-${name}.json`), JSON.stringify(redactDeep(payload), null, 2));
  } catch {
    // best effort
  }
}
...
export function writeKeyed(config, dir, key, payload) {
  try {
    ...
    privateWrite(join(config.dataDir, dir, `${String(key).replace(/[^a-zA-Z0-9_-]/g, "_")}.json`), JSON.stringify(redactDeep(payload), null, 2));
  } catch {
    // best effort
  }
}
...
export function cacheSet(config, key, value) {
  try {
    ...
    privateWrite(join(config.dataDir, "cache", key), JSON.stringify(value));
  } catch {
    // best effort
  }
}
...
export function clip(text, max) {
  ...
  // Redact before cutting: a token cut in half no longer matches its shape.
  text = redact(text);
...
export function writeMarker(config, sessionId, data) {
  try {
    ...
    privateWrite(markerPath(config, sessionId), JSON.stringify(data));
  } catch {
    // best effort
  }
}
